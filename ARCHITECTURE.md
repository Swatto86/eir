...
- `analyze_with` wraps each provider call in a bounded transient-retry loop (`MAX_AI_RETRIES = 2`, 2s/4s backoff) gated by `is_transient_ai_error` (HTTP 429/5xx/overload/timeout/connection markers). Since v0.30.1 a CLI subprocess (Claude/Kilo) that exits non-zero with **empty stderr** is also transient: `call_claude_cli`/`call_kilo_cli` emit a distinct "…and no error output (transient)" message so a swallowed API/overload blip retries instead of latching `st.error` red, while an exit that *did* print a diagnostic (auth/config) still fails immediately.
...
- `restart_self`: spawns a detached PowerShell helper (LocalSystem, no UAC, survives this process exiting) that stops EirSvc, waits for STOPPED (≤60s; a missing service also exits the wait), then retries `sc start` every 5s over a 60s window checking for Running each second; all helper streams redirect to `eir-restart.log` next to the exe (the only evidence when a restart fails to come back). Returns `bool`: a helper that fails to spawn means nothing would ever start the service again, so the `UpdateSettings` caller stays alive on the old settings instead of stopping into the void (v0.27.5 — hardened after a live incident where the service stopped and never returned).
...
`AiClient` (`ai/client.rs`) wraps a single `reqwest::Client` (300s timeout, set for slow free OpenRouter models), the normalised reasoning `effort` string, and an internal `enum AiClientConfig` with one variant per provider: two native HTTP backends (`Anthropic`, `OpenRouter`) and two subprocess backends (`ClaudeCli`, `KiloCli`) that borrow a locally logged-in CLI session instead of taking a pasted key (only the OpenAI-compatible proxy stays removed — its legacy config value aliases to Anthropic on load). **v0.19 removed the API-key-based `KiloCode` gateway provider** (`api.kilo.ai`) in favour of `KiloCli`, which reached the same subscription/BYOK routing through the already-working `claude_cli`-style subprocess pattern; a `provider = "kilocode"` (or the legacy `"kilo"` alias) in an old config now loads as `kilo_cli`. **OpenRouter is now the only provider that takes a pasted API key** (the default `anthropic` provider also needs one but predates this cleanup and is unaffected by it):
...
- **`ClaudeCli`** — Claude on the user's **subscription**: spawns the local `claude` binary (`--print --output-format json`, optional `--model`, `--effort`), **no API key**. `resolve_user_profile` uses the configured profile or scans `C:\Users\*\.claude\.credentials.json`; `resolve_claude_binary` tries the configured path, then `<profile>\.local\bin\claude.exe` (native installer), then npm's install layout under the profile — `AppData\Roaming\npm\node_modules\@anthropic-ai\claude-code\bin\claude.exe` (the exe the `claude.cmd` shim dispatches to), then the `claude.cmd` shim itself (npm's stable entry point, covering package-layout drift) — then PATH; the full-path candidates matter because LocalSystem has no per-user PATH, so a bare `claude` that works interactively fails as a service (live-hit in v0.27.4: an npm-installed CLI resolved to bare `claude` and every spawn failed). When running as LocalSystem it injects `USERPROFILE`/`APPDATA`/etc. so the CLI finds the user's session; `kill_on_drop(true)` reaps the large process on the 300s timeout. Usage comes from the CLI's JSON envelope (`result`, `total_cost_usd`, cache-token fields — the cost is the *equivalent* API cost; the call itself is covered by the subscription, and the UI shows "—"/no-charge). Blank model = the CLI's configured default.
- **`KiloCli`** — Kilo Code on the user's **subscription** (Kilo Pass / Token-Plan addons / BYOK): spawns the local `kilo` binary (`run --auto --format json --agent ask`, optional `-m`/`--variant`), **no API key**. `resolve_kilo_cli_profile` scans `C:\Users\*\.local\share\kilo\auth.json` — the CLI's actual session file, a JSON object keyed by provider name (`"kilo"` for the Pass/Token-Plan login, plus any BYOK provider entries). `resolve_kilo_cli_binary` mirrors `resolve_claude_binary`'s structure but for npm's install layout: it tries the configured path, then the platform-specific exe under the resolved profile's `AppData\Roaming\npm\node_modules\@kilocode\cli\node_modules\@kilocode\{cli-windows-x64,cli-windows-x64-baseline}\bin\kilo.exe` (Windows npm installs resolve to either variant depending on CPU feature detection at install time), then the npm shim `kilo.cmd`, then falls back to bare `"kilo"` on PATH — this matters because the LocalSystem service account has no per-user PATH, so the bare-name fallback that works fine when run interactively silently fails as a service. `call_kilo_cli` injects the same `USERPROFILE`/`APPDATA`/etc. env vars as `call_claude_cli` plus `HOME` (the kilo CLI is a Bun-compiled binary; Bun tooling can consult `HOME` on Windows). Output is NDJSON (`parse_kilo_ndjson`): each event nests its real payload under a `part` object (e.g. `{"type":"text","part":{"text":"..."}}`) — `text` events concatenate into the reply and the last `step_finish` event's `part.tokens`/`part.cost` become usage, with a fallback to the equivalent top-level fields for older/alternate event shapes. **Models routed through the subscription/Token-Plan/BYOK require a `kilo/` prefix** (e.g. `kilo/minimax/minimax-m2.5`); a bare `provider/model` id (e.g. `minimax/minimax-m3`) instead routes to that provider directly and fails without the user's own credentials there — Eir does not auto-prefix, a wrong model string surfaces the CLI's own error.

Both SSE readers share `SseLineBuf`, a byte-based line accumulator (unit-tested), so a multi-byte UTF-8 character split across network chunks can never drop data. The CLI envelope decode is unit-tested (`claude_cli_envelope_parses_usage`); the NDJSON decode is unit-tested (`kilo_ndjson_collects_text_and_keeps_last_step_usage`, `kilo_ndjson_tolerates_garbage_lines_and_alternate_token_shape`).
...
Native candidate identity is anchored to the machine (`native_candidates_from`, `check.rs:291`): an AI-reported update whose name doesn't resolve to an actually-installed app (`match_installed` against the merged winget + registry inventory set) is dropped, and only strictly-newer versions (`version::is_newer`, with the marketing-truncation guard) survive — preventing a poisoned AI from naming a fabricated app to pick an arbitrary vendor domain.
...
The unmanaged AI check is capped at `AI_CHECK_CAP` (20) apps per cycle to bound cost, but without rotation the same front of the list would be re-checked forever and the tail never reached. A new `update_checks` table (migration 0016) records the last AI-check time per app. `check_unmanaged` sorts the merged inventory stalest-first (`history::last_check_times`), takes the cap, records a check for each one (`history::record_check`), and only then asks the AI. This guarantees every app in the unmanaged inventory gets reached across cycles even when the list is far larger than the per-cycle cap.
