...
Per-app AI guidance uses `[updater.notes]`; guidance proven by a successful targeted retry is mirrored in `[updater.learned_notes]`. The App Updates view can create guidance from a detected row and read/edit/delete it later from the persistent list. The stable, version-stripped installed identity—not an AI-returned alias—is the key. Exact guidance is preferred; learned guidance may be reused only when compact product ids contain one another (for example `tool` and `tool x64`). It is included in targeted version checks, retry diagnosis, and native-installer prompts. After success, an AI rewrite is accepted only if shorter, within the existing 2,000-character cap, and every explicit original URL remains present; otherwise the whitespace-normalised original becomes the proven note. A user edit/delete removes proven status. None of this can relax Rust's URL, hash, Authenticode, archive-size, or silent-install gates. Service-side limits cap ids at 200 characters and guidance maps at 500 entries.
