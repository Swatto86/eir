...
- Game Mode auto-detect (`game_detect.rs`): every 5 s the tray asks `SHQueryUserNotificationState` whether a fullscreen app other than Eir is in front and sends `SetGaming { manual: false }`, re-asserted every 30 s against the service's 90 s lease. It holds the fullscreen process open (`PROCESS_SYNCHRONIZE`) so it can tell a break from the end of a session: while that process still runs, Game Mode survives up to 10 minutes out of fullscreen (`AWAY_GRACE`); once it has exited, or cannot be opened (an elevated game), it ends after 60 s (`EXIT_DEBOUNCE`). Leaving Game Mode restores the power plan and lets deferred analysis and updates run, and the 60-second rule alone dropped it several times an hour whenever the owner alt-tabbed out of WoW. The timing rules are the pure `Detector`.
...
- **`actionable_fingerprint`** decides whether a cycle is even worth an AI call and dedups unchanged states. It is built from **shared predicates in `models.rs`** so the fingerprint and the reactive triggers can't drift: `LogEvent::is_actionable()` (`F|path|sev|<hash of the new lines>` — the watcher reports only newly written lines, so two different new errors in one file are a change), `EventLogEntry::is_actionable()` (`E|level|source|id`), and `SystemState::fault_parts()` (`S|name`, `FW|name` only when explicitly `Some(false)`, `DEF|realtime_off`/`DEF|sig_stale` for age `>3` only when Defender is the active AV), plus CPU/MEM/DISK `>90` flags. Parts are sorted and joined; **empty → `None` → skip the Claude call.** Identical fingerprint across cycles means nothing changed, so it's skipped.
...
2. **Collect candidates** — `check::collect`. Each available manager lists its updates (`winget upgrade --include-unknown`, `choco outdated -r`, `winget upgrade --source msstore`). Results are de-duplicated by app identity via `Collected::push`: earlier (more-preferred) managers win, the id is `clean_app_name(name).to_lowercase()`, `should_skip` / the seen-set drop ignored/duplicate apps, and a Chocolatey package and its `.install`/`.portable`/… variant are one app (`vlc` and `vlc.install` were both upgraded in one cycle; upgrading the first brings the second). An app whose installed version is unknown (winget's "Unknown", or blank) is left to update itself and named in a note: winget refuses to upgrade it without `--include-unknown` and no update of it could be verified, so Battle.net failed in every daily cycle for three weeks. When a primary manager handles the app, the native installer is appended as a self-healing fallback method (unless the primary already is native). Then an **AI web-search pass** over apps no manager covers (`check_unmanaged`, `check.rs:244`) produces native-only candidates. Both this discovery pass and the native-installer lookup go first to GitHub's canonical `/owner/repo/releases/latest` redirect for GitHub-hosted software, avoiding guessed tags or stale generic release pages. The unmanaged inventory is now independent of winget: `inventory::list_installed` (`inventory.rs`) enumerates the HKLM + Wow6432Node + per-user Uninstall registry keys, and the result is merged with `winget list` when winget is present. If winget is missing or the AI/native path is disabled, a UI note explains the degraded coverage instead of staying silent. Apps registered only in a user's hive (HKCU/HKU, or `ARP\User\…` in `winget list`) are excluded from the native path and named in a note: they live in the user's profile, so their installer run as SYSTEM would install a second copy for SYSTEM (and the owner's per-user Tauri and Electron apps keep themselves current). One machine-wide registration makes an app machine-wide. Apps with an unknown installed version are not sent to the AI check either.
3. **Pause repeated failures** — `backoff::paused_apps` over `history::attempt_records` (attempts since the last Clear, within 60 days). An app whose update failed in consecutive runs is not tried on its own for 2 days after two failed runs, 4 after three, then a week; one success or "already current" ends the streak. Paused apps are listed as failed rows (not recorded as attempts) whose detail gives the run count, the next automatic date and the last real failure; Retry on the row runs the targeted retry at once. On the owner's PC the same apps (no silent installer, unsigned download, vendor web page instead of an installer) failed every day and cost an AI web search each time.
4. **Heal each candidate** (bounded by `max_apps_per_run` and `max_attempts_per_app`) — `heal` (`orchestrator.rs:134`).
5. **Record** every attempt to the `update_attempts` table under `cycle_id` (`history::record_attempts`).
...
- Apps installed only for a user, and apps with no known installed version, are never
  updated by Eir unless a package manager (winget, running as the desktop user) handles
  them; they are named in the run's notes. A paused app waits up to a week unless Retry is
  used, even if its vendor ships a newer release meanwhile, because the target version of a
  failed attempt is not recorded.
- The native path still needs a direct installer link from the AI. Vendors whose download
  is a web page, a redirect to another host, or an unsigned installer (the owner's own
  apps are not Authenticode-signed) fail by design and are then paused.
