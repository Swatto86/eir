...
mod windows {
    ...
    fn build_set_enabled_script(key: &str, name: &str, enable: bool, source_guard: &str) -> String {
        let key_q = crate::executor::powershell::ps_single_quote(key);
        let name_q = crate::executor::powershell::ps_single_quote(name);
    ...
    fn build_source_guard(location: &str, hive: &str, name: &str) -> Result<String> {
        ...
        if matches!(location, "common_startup_folder" | "user_startup_folder") {
            ...
            let base = if location == "common_startup_folder" {
                ...
                let profile_q = crate::executor::powershell::ps_single_quote(&profile_key);
                format!(
    ...
    pub async fn set_enabled(
        ...
        if location == "scheduled_task" {
            return if enable {
                crate::executor::tasks::enable(name).await
            } else {
                crate::executor::tasks::disable(name).await
            };
        }
        ...
        crate::executor::powershell::run_diagnostic(&script).await
    }
...
#[cfg(windows)]
pub(crate) use windows::{active_user_sid, set_enabled, valid_sid};

/// No interactive-user-SID concept exists on headless Linux — fail closed so any
/// caller reached by shared (`startup_scan.rs`) code refuses rather than treating an
/// arbitrary string as a valid identity.
#[cfg(unix)]
pub(crate) fn valid_sid(_sid: &str) -> bool {
    false
}

/// Startup-entry management (the Windows "StartupApproved" registry mechanism) has no
/// Linux equivalent — `FixAction::StartupSet` is hard-blocklisted on Linux (see
/// `executor::mod::linux_unsupported` and `policy.linux.toml`), so this is never
/// called from the executor dispatch; it exists purely so `active_user_sid()`'s many
/// OS-neutral callers in `main.rs` (per-user log-directory discovery/reconciliation)
/// keep compiling unchanged and simply find no stable active user on a headless box.
#[cfg(unix)]
pub(crate) fn active_user_sid() -> Result<String> {
    anyhow::bail!("Startup-entry / active-user-SID tracking is not available on Linux")
}
