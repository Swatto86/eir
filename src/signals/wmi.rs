...
fn get_network_interfaces() -> Vec<NetworkInterface> {
    ...
    while !adapter_ptr.is_null() {
        ...
        let status = if ipv4.is_some() { "up" } else { "down" };
...
fn snapshot_state() -> SystemState {
    ...
    SystemState {
        uptime_secs,
        cpu_usage_percent: 0.0, // Accurate CPU % requires two samples; placeholder for Phase 1
            ...
            tokio::select! {
                _ = ticker.tick() => {
                    match tokio::task::spawn_blocking(snapshot_state).await {
                        Ok(s) => {
                            info!(
                                mem_pct = s.memory_usage_percent,
                                disk_pct = s.disk_usage_percent,
