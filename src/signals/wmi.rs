...
use windows::Win32::System::Services::{
    ...
    SC_MANAGER_ENUMERATE_SERVICE, SERVICE_ACTIVE, SERVICE_RUNNING, SERVICE_WIN32,
...
fn get_memory() -> (f32, f32) {
    ...
    unsafe { let _ = GlobalMemoryStatusEx(&mut mem); }
...
fn get_services() -> (usize, Vec<String>) {
    ...
    unsafe {
        let _ = EnumServicesStatusExW(
            manager, SC_ENUM_PROCESS_INFO, SERVICE_WIN32, SERVICE_ACTIVE,
            None, &mut bytes_needed, &mut services_returned,
            Some(&mut resume_handle), PCWSTR::null(),
        );
    }
...
fn get_network_interfaces() -> Vec<NetworkInterface> {
    ...
    let result = unsafe {
        GetAdaptersInfo(Some(buf.as_mut_ptr() as *mut IP_ADAPTER_INFO), &mut buf_size)
    };
    ...
    while !adapter_ptr.is_null() {
        ...
        let ipv4 = if ip == "0.0.0.0" || ip.is_empty() { None } else { Some(ip) };
