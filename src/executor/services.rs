...
use windows::Win32::System::Services::{
    ...
    SERVICE_RUNNING, SERVICE_START, SERVICE_STATUS, SERVICE_STATUS_CURRENT_STATE,
...
pub fn stop(name: &str) -> Result<String> {
    ...
    let svc = unsafe { OpenServiceW(manager, PCWSTR(name_w.as_ptr()), SERVICE_STOP | SERVICE_QUERY_STATUS) };
...
fn wait_for(name: &str, target: SERVICE_STATUS_CURRENT_STATE, timeout_secs: u64) -> Result<()> {
    ...
    loop {
        if Instant::now() > deadline {
            unsafe { let _ = CloseServiceHandle(svc); let _ = CloseServiceHandle(manager); }
